...
export default {
  ...
  methods: {
    ...
    handleExport() {
      if (this.total > 10000) {
        this.$message.warning('数据量较大，导出可能需要较长时间');
      }
    ...
    buildRequestParams() {
      ...
      if (this.queryForm.college) params.college = String(this.queryForm.college);
      if (this.queryForm.major) params.major = String(this.queryForm.major);
      if (this.queryForm.grade) params.grade = String(this.queryForm.grade);
      if (this.queryForm.class) params.className = String(this.queryForm.class);
      if (this.queryForm.studentId) params.studentId = String(this.queryForm.studentId);
    ...
    processStatsData(response) {
      if (response && response.data) {
        ...
        this.avgDaily = data.average_consumption || data.avg_daily || 0;
    ...
    processTableData(response) {
      if (response && response.data) {
        ...
        if (data.records && Array.isArray(data.records)) {
          ...
          this.total = data.total || data.totalCount || data.records.length;
        } else if (data.list && Array.isArray(data.list)) {
          this.tableData = data.list;
          this.total = data.total || data.totalCount || data.list.length;
        } else if (Array.isArray(data)) {
          this.tableData = data;
          this.total = data.length;
        } else if (Array.isArray(data.data)) {
          this.tableData = data.data;
          this.total = data.total || data.totalCount || data.data.length;
        } else {
          this.tableData = [];
          this.total = 0;
        }
