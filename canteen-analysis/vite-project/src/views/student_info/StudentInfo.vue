...
const handleQuery = async () => {
  ...
  try {
    ...
    const result = await getStudentInfo(queryParams)
    console.log('API返回数据:', result)

    // 统一处理数据提取逻辑
    // 根据后端API，可能返回分页数据或直接返回数组
    let records = []
    if (result?.data?.records) {
      ...
      if (result.data.total !== undefined) {
        ...
        pagination.value.total = result.data.totalCount
      } else {
    ...
    studentList.value = records.map(item => ({
      ...
      gender: item.gender_display || item.gender || item.gender_display,
      ...
      phone: item.phone_number || item.phone || item.telephone || item.contact_phone || ''
    }))
  } catch (error) {
...
const showScoreDetail = async (student) => {
  ...
  try {
    ...
    if (result && result.data) {
      ...
      // 处理不同的响应格式
      let gpaValue = 0
      let termDisplay = getSemesterDisplay(selectedSemester.value)
      
      if (typeof backendData === 'object') {
        // 如果是对象，尝试多种字段名
        gpaValue = parseFloat(backendData.gpa || backendData.GPA || backendData.grade_point || 0)
        termDisplay = backendData.term_display || backendData.termDisplay || backendData.semester || termDisplay
      } else if (typeof backendData === 'number') {
        // 如果直接是数字，可能是GPA
        gpaValue = parseFloat(backendData)
      }

      // 更新当前GPA
      currentGPA.value = gpaValue

      // 清空趋势图数据（单次请求只有当前学期数据）
      scoreData.value = {
        student: student.name,
        term: selectedSemester.value,
        term_display: termDisplay,
        gpa: currentGPA.value,
        gpaTrend: []  // 单次请求无法获取历史趋势
      }
...
const initGPAChart = (gpaTrend) => {
  ...
  const semesterLabels = generateSemesterLabels(gpaTrend.length)
  const option = {
    tooltip: {
      trigger: 'axis',
      formatter: (params) => {
...
<template>
  <div class="student-info">
    ...
    <div class="table-card">
      <el-table :data="paginatedStudentList" v-loading="loading">
